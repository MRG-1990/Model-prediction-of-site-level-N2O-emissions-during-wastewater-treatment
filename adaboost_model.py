"""
AdaBoost workflow refactored from Scenario_1.ipynb.
...
def run_hyperband_search(
    ...
    """
    Perform a Hyperband-style search to find a strong AdaBoost configuration.
    """
    max_iter = 81
    eta = 3
    ...
    for s in reversed(range(s_max + 1)):
        ...
        for _ in range(s + 1):
            ...
            for config in candidates:
                base_estimator = DecisionTreeRegressor(max_depth=config["max_depth"])
...
def plot_learning_curve_adaboost(
    model,
    x_train: pd.DataFrame,
    y_train: pd.Series,
    output_path: str,
...
def main():
    ...
    if USE_HYPERBAND:
        best_params, _ = run_hyperband_search(x_train_clean, y_train_clean)
    else:
        print("\n" + "=" * 60)
        print("1. Hyperparameter Optimization Using Hyperband")
        print("=" * 60)
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(x_train_clean), 1):
        x_train_fold = x_train_clean.iloc[train_index]
        x_val_fold = x_train_clean.iloc[val_index]
        y_train_fold = y_train_clean.iloc[train_index]
        y_val_fold = y_train_clean.iloc[val_index]
    ...
    plot_learning_curve_adaboost(
        model=model,
        x_train=x_train,
        y_train=y_train,
        output_path="adaboost_learning_curve.jpg",
    )
    ...
    plot_builtin_feature_importance(
        ...
        title="Feature Importance in the AdaBoost Algorithm",
        output_path="adaboost_feature_importance.jpg",
    )
    ...
    plot_residuals_over_time(
        ...
        title="AdaBoost over Time",
        y_label="N₂O Residuals (g-N/day)",
        output_path="adaboost_residuals_jointplot.jpg",
    )
    ...
    plot_zero_bias_regression(
        ...
        x_label="Actual N₂O emissions (g-N/day)",
        y_label="Predicted N₂O emissions (g-N/day)",
        output_path="adaboost_zero_bias_regression.jpg",
    )
