"""
SVR workflow refactored from Scenario_1.ipynb.
...
def sample_random_params() -> dict:
    ...
    return {
        ...
        "epsilon": round(random.uniform(0.01, 0.1), 3),
...
def run_hyperband_search(
    ...
    """
    Perform a Hyperband-style search to find a strong SVR configuration.
    """
    max_iter = 81
    eta = 3
...
def plot_learning_curve_svr(
    ...
    plt.ylabel("Score (R²)")
    plt.ylim(0.90, 0.94)
    plt.title("Learning Curve - SVR (Support Vector Regression)")
    plt.legend()
    plt.savefig(output_path, format="jpg", dpi=300, bbox_inches="tight")
    plt.show()
...
def plot_permutation_importance(
    ...
    importance_data = pd.DataFrame({"name": feature_names, "importance": importances_mean})
    importance_data = importance_data.sort_values(by="importance", ascending=False)
    importance_data["importance"] = (
        importance_data["importance"] / importance_data["importance"].sum()
    ) * 100

    fig, ax = plt.subplots(figsize=(10, 8))
    sns.barplot(data=importance_data, x="importance", y="name")

    for idx, row in enumerate(importance_data.itertuples()):
        ax.text(row.importance, idx, f"{row.importance:.1f}%", va="center")
...
def main():
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(x_train), 1):
        ...
        print(f"Fold {fold} - Mean Squared Error (MSE): {mse:.2f}")
        print(f"Fold {fold} - R-squared (R²): {r2:.2f}")
        print(f"Fold {fold} - Mean Absolute Error (MAE): {mae:.2f}\n")
    ...
    plot_learning_curve_svr(
        model=model,
        x_train=x_train,
        y_train=y_train,
        output_path="svr_learning_curve.jpg",
    )
    ...
    plot_permutation_importance(
        ...
        title="Feature Importance in the SVR Algorithm",
        output_path="svr_feature_importance.jpg",
    )
    ...
    plot_residuals_over_time(
        ...
        output_path="svr_residuals_jointplot.jpg",
    )
    ...
    plot_zero_bias_regression(
        ...
        output_path="svr_zero_bias_regression.jpg",
    )
